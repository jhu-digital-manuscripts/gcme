...
const null_leader_1 = require("./null-leader");
...
class LeaderChooser {
    ...
    register(addon, create) {
        ...
        if (!(0, semver_1.satisfies)(addon.pkg.version, '>=2.0.0-alpha.0', {
        ...
        if ((0, shared_internals_1.isDeepAddonInstance)(addon)) {
            ...
            let range = (_a = dependencies === null || dependencies === void 0 ? void 0 : dependencies['ember-auto-import']) !== null && _a !== void 0 ? _a : devDependencies === null || devDependencies === void 0 ? void 0 : devDependencies['ember-auto-import'];
    ...
    get leader() {
        if (!this.locked) {
            if (!this.appCandidate) {
                return new null_leader_1.NullLeader(`To use these addons, your app needs ember-auto-import >= 2: ${this.addonCandidates
                    .map((c) => c.parentName)
                    .sort()
                    .join(', ')}`);
            }
            let eligible = [this.appCandidate, ...this.addonCandidates].filter((c) => (0, semver_1.satisfies)(c.version, this.appCandidate.range, {
                includePrerelease: true,
            }));
            if (eligible.length === 0) {
                throw new Error(`ember-auto-import was unable to find any copy of itself that satisfies the app's requested range of ${this.appCandidate.range}`);
            }
            let [candidate, ...rest] = eligible;
            for (let challenger of rest) {
                if ((0, semver_1.gt)(challenger.version, candidate.version)) {
                    candidate = challenger;
                }
            }
            this.locked = candidate.create();
            debug('elected %s from %s which satisfies %s', candidate.version, 'parentName' in candidate ? candidate.parentName : 'the app', this.appCandidate.range);
            let v1 = g[protocolV1];
