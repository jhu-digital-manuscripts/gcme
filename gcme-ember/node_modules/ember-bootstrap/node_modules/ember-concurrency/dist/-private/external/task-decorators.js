...
function taskFromPropertyDescriptor(target, key, descriptor, params = [], factoryClass = TaskFactory$1) {
  ...
  let taskFn;
  deprecate('Using @task decorator (or any of its variants) is deprecated. Please use modern `taskName = task(async () => {})` syntax instead', false, {
...
const lastValue = decoratorWithParams((_target, _key, descriptor, [taskName] = []) => {
  ...
  deprecate('@lastValue is deprecated is deprecated along with all ember-concurrency decorators. Please use a getter that references taskName.lastSuccessful.value instead.', false, {
