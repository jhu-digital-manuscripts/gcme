...
class BasicDropdown extends Component {
  ...
  constructor(owner, args) {
    ...
    if (this.args.dropdownId !== undefined) {
      deprecate('You have passed `@dropdownId` into `ember-basic-dropdown`. This property does not work correctly without custom modifiers and is undocumented. Remove this parameter and use the `uniqueId` property from the public API instead.', false, {
  ...
  _getDestinationId() {
    ...
    if (!_configSet) {
      ...
      if (configEnvironment['APP']?.rootElement) {
        deprecate(`ember-basic-dropdown received the \`APP.rootElement\` value from \`ember-cli-build.js\`. You now need to pass this value using \`import { setConfig } from 'ember-basic-dropdown/config'; setConfig({rootElement: config.APP['rootElement']});`, false, {
          for: 'ember-basic-dropdown',
          id: 'ember-basic-dropdown.config-environment',
          since: {
            enabled: '8.9',
  ...
  get contentComponent() {
    if (this.args.contentComponent) {
      return ensureSafeComponent(this.args.contentComponent, this);
    }
