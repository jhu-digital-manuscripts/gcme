...
(function (ember, emberGlimmer, templateCompilation, emberApp) {
          ...
          */
          ...
          const emberDebugIndex = /*#__PURE__*/Object.defineProperty({
                    ...
                    get assert () { return assert$2; },
                    captureRenderTree,
                    get debug () { return debug$1; },
          ...
          */
          ...
          function buildElement$1({
            ...
            if (attrs) {
              let {
                params} = buildElementParams(attrs, symbols);
              out.push(...params);
            }
          ...
          function buildVar$1(head, context, symbols, path) {
            ...
            if (path === undefined || path.length === 0) {
              ...
              assert$1(op !== opcodes.GetStrictKeyword);
              return [op, sym, path];
            }
          }
          ...
          */
          ...
          class Matcher {
            ...
            matchFor(left, right) {
              ...
              assert$1(isPresentArray(nesteds));
              ...
              while (seenChars < sourceLength) {
                ...
                if (seenLines === line - 1) {
                  ...
                  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
                  {
                    ...
                    assert$1(roundTrip.line === line);
                    assert$1(roundTrip.column === column);
                  }
          ...
          function buildBlock(path, params, hash, _defaultBlock, _elseBlock = null, loc, openStrip, inverseStrip, closeStrip) {
            ...
            if (_elseBlock?.type === 'Template') {
              assert$1(_elseBlock.blockParams.length === 0);
          ...
          function buildElement(tag, options = {}) {
            ...
            if (typeof tag === 'string') {
              ...
              assert$1(tag.type === 'PathExpression', `Invalid tag type ${tag.type}`);
              path = tag;
            } else if ('path' in tag) {
              // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- supports JS users
              assert$1(tag.path.type === 'PathExpression', `Invalid tag type ${tag.path.type}`);
              path = tag.path;
          ...
          class Parser {
            ...
            get currentTag() {
              ...
              assert$1(node && (node.type === 'StartTag' || node.type === 'EndTag'));
              return node;
            }
            get currentStartTag() {
              let node = this.currentNode;
              assert$1(node && node.type === 'StartTag');
              return node;
            }
            get currentEndTag() {
              let node = this.currentNode;
              assert$1(node && node.type === 'EndTag');
              return node;
            }
            get currentComment() {
              let node = this.currentNode;
              assert$1(node && node.type === 'CommentStatement');
              return node;
            }
            get currentData() {
              let node = this.currentNode;
              assert$1(node && node.type === 'TextNode');
              return node;
            }
            acceptNode(node) {
            ...
            parseProgram(node, program) {
              ...
              if (node !== poppedNode) {
                if (poppedNode?.type === 'ElementNode') {
                  ...
                  assert$1(false, `[BUG] mismatched parser elementStack node: ${node.type}`);
                }
              }
              return node;
            }
          ...
          class TokenizerEventHandlers extends HandlebarsNodeVisitors {
            ...
            finishTag() {
              ...
              if (tag.type === 'StartTag') {
                ...
                assert$1(tag.type === 'EndTag', `Invalid tag type ${tag.type}`);
            ...
            finishEndTag(isVoid) {
              ...
              if (isVoid) {
                ...
                assert$1(element.closeTag === null);
              } else {
            ...
            parsePossibleBlockParams() {
              ...
              assert$1(this.tokenizer.state === ATTRIBUTE_NAME);
              ...
              const handlers = {
                PossibleAs: next => {
                  assert$1(state.state === 'PossibleAs');
                  if (isSpace(next)) {
                    // " as ..."
                    state = {
                ...
                BeforeStartPipe: next => {
                  assert$1(state.state === 'BeforeStartPipe');
                  if (isSpace(next)) {
                    this.tokenizer.consume();
                  } else if (next === '|') {
                    state = {
                ...
                BeforeBlockParamName: next => {
                  assert$1(state.state === 'BeforeBlockParamName');
                  if (isSpace(next)) {
                    this.tokenizer.consume();
                  } else if (next === '') {
                ...
                BlockParamName: next => {
                  assert$1(state.state === 'BlockParamName');
                  if (next === '') {
                    // The HTML tokenizer ran out of characters, so we are either
                ...
                AfterEndPipe: next => {
                  assert$1(state.state === 'AfterEndPipe');
                  if (isSpace(next)) {
                    this.tokenizer.consume();
                  } else if (next === '') {
                    // The HTML tokenizer ran out of characters, so we are either
          ...
          */
          ...
          class ExpressionEncoder {
            ...
            PathExpression({
              ...
              assert$1(getOp[0] !== opcodes.GetStrictKeyword);
