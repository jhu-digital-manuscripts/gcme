...
class Matcher {
  ...
  matchFor(left, right) {
    ...
    assert$2(isPresentArray(nesteds));
    ...
    while (seenChars < sourceLength) {
      ...
      if (seenLines === line - 1) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
        {
          ...
          assert$2(roundTrip.line === line);
          assert$2(roundTrip.column === column);
        }
...
function buildBlock(path, params, hash, _defaultBlock, _elseBlock = null, loc, openStrip, inverseStrip, closeStrip) {
  ...
  if (_elseBlock?.type === 'Template') {
    assert$2(_elseBlock.blockParams.length === 0);
...
function buildElement(tag, options = {}) {
  ...
  if (typeof tag === 'string') {
    ...
    assert$2(tag.type === 'PathExpression', `Invalid tag type ${tag.type}`);
    path = tag;
  } else if ('path' in tag) {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- supports JS users
    assert$2(tag.path.type === 'PathExpression', `Invalid tag type ${tag.path.type}`);
    path = tag.path;
...
class Parser {
  ...
  get currentTag() {
    ...
    assert$2(node && (node.type === 'StartTag' || node.type === 'EndTag'));
    return node;
  }
  get currentStartTag() {
    let node = this.currentNode;
    assert$2(node && node.type === 'StartTag');
    return node;
  }
  get currentEndTag() {
    let node = this.currentNode;
    assert$2(node && node.type === 'EndTag');
    return node;
  }
  get currentComment() {
    let node = this.currentNode;
    assert$2(node && node.type === 'CommentStatement');
    return node;
  }
  get currentData() {
    let node = this.currentNode;
    assert$2(node && node.type === 'TextNode');
    return node;
  }
  acceptNode(node) {
  ...
  parseProgram(node, program) {
    ...
    if (node !== poppedNode) {
      if (poppedNode?.type === 'ElementNode') {
        ...
        assert$2(false, `[BUG] mismatched parser elementStack node: ${node.type}`);
      }
    }
    return node;
  }
...
class TokenizerEventHandlers extends HandlebarsNodeVisitors {
  ...
  finishTag() {
    ...
    if (tag.type === 'StartTag') {
      ...
      assert$2(tag.type === 'EndTag', `Invalid tag type ${tag.type}`);
  ...
  finishEndTag(isVoid) {
    ...
    if (isVoid) {
      ...
      assert$2(element.closeTag === null);
    } else {
  ...
  parsePossibleBlockParams() {
    ...
    assert$2(this.tokenizer.state === ATTRIBUTE_NAME);
    ...
    const handlers = {
      PossibleAs: next => {
        assert$2(state.state === 'PossibleAs');
        if (isSpace(next)) {
          // " as ..."
          state = {
      ...
      BeforeStartPipe: next => {
        assert$2(state.state === 'BeforeStartPipe');
        if (isSpace(next)) {
          this.tokenizer.consume();
        } else if (next === '|') {
          state = {
      ...
      BeforeBlockParamName: next => {
        assert$2(state.state === 'BeforeBlockParamName');
        if (isSpace(next)) {
          this.tokenizer.consume();
        } else if (next === '') {
      ...
      BlockParamName: next => {
        assert$2(state.state === 'BlockParamName');
        if (next === '') {
          // The HTML tokenizer ran out of characters, so we are either
      ...
      AfterEndPipe: next => {
        assert$2(state.state === 'AfterEndPipe');
        if (isSpace(next)) {
          this.tokenizer.consume();
        } else if (next === '') {
          // The HTML tokenizer ran out of characters, so we are either
...
class ExpressionEncoder {
  ...
  PathExpression({
    ...
    assert$2(getOp[0] !== opcodes.GetStrictKeyword);
